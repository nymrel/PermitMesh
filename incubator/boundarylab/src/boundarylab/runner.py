...
def _results_for_outcome(
    ...
    *,
    ...
    if "network.exact_targets" in profile.probes:
        ...
        results = [
            replacement if item.probe_id == replacement.probe_id else item
            for item in results
        ]
    return results
    ...
    *,
    ...
    if adapter_name not in authorization.allowed_adapters:
        raise BoundaryLabError(
            f"adapter {adapter_name!r} is not authorized by this manifest"
        )
    if allow_network_probes and not authorization.network_targets:
        raise BoundaryLabError(
            "network probes require at least one exact authorized target"
        )
    if allow_network_probes:
        ...
        raise BoundaryLabError(
            "--allow-public-network requires --allow-network-probes"
        )
