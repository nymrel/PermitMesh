...
def _host(value: JSONValue, label: str) -> str:
    ...
    if any(token in text for token in ("*", "/", "://", "@", " ", "\t", "\n")):
        raise BoundaryLabError(
            f"{label} must be one exact hostname or IP address"
        )
    try:
...
class NetworkTarget:
    ...
    def from_json(cls, value: JSONValue) -> "NetworkTarget":
        ...
        if expected not in {"allowed", "blocked"}:
            raise BoundaryLabError(
                "network target.expected must be allowed or blocked"
            )
        return cls(
...
class AdapterConfig:
    ...
    def from_json(cls, value: JSONValue) -> "AdapterConfig":
        ...
        if "command_prefix" in data:
            ...
            prefix = tuple(
                _string(item, "command argument", maximum=256)
                for item in items
            )
        image = _string(
        ...
        if any(char.isspace() for char in image):
            raise BoundaryLabError(
                "adapter_config.docker_image may not contain whitespace"
            )
...
class Authorization:
    ...
    def from_json(cls, value: JSONValue) -> "Authorization":
        ...
        if data["schema"] != AUTH_SCHEMA:
            raise BoundaryLabError(
                f"authorization.schema must equal {AUTH_SCHEMA}"
            )
        ...
        if environment not in {"local", "development", "test"}:
            raise BoundaryLabError(
                "authorization.environment must be local, development, or test"
            )
        ...
        adapters = tuple(
            _string(item, "adapter", maximum=32) for item in adapter_values
        )
        if not adapters or len(adapters) != len(set(adapters)):
            raise BoundaryLabError(
                "authorization.allowed_adapters must be a non-empty unique list"
            )
        unknown_adapters = sorted(set(adapters) - ADAPTERS)
        if unknown_adapters:
            raise BoundaryLabError(
                f"unsupported adapters: {', '.join(unknown_adapters)}"
            )
        targets = tuple(
    ...
    def to_json(self) -> dict[str, JSONValue]:
        return {
            ...
            "network_targets": [
                target.to_json() for target in self.network_targets
            ],
            "owner": self.owner,
            "schema": AUTH_SCHEMA,
...
class Profile:
    ...
    def from_json(
        ...
        probes = tuple(
            _string(item, "profile probe", maximum=128)
        ...
        if not probes or len(probes) != len(set(probes)):
            raise BoundaryLabError(
                "profile.probes must be a non-empty unique list"
            )
        unknown = sorted(set(probes) - known_probes)
        if unknown:
            raise BoundaryLabError(
                f"profile references unknown probes: {', '.join(unknown)}"
            )
        ...
        if outside:
            raise BoundaryLabError(
                "expectations reference probes outside the profile: "
                + ", ".join(outside)
            )
        expectations = {
    ...
    def to_json(self) -> dict[str, JSONValue]:
        return {
            ...
            "expectations": {
                key: value.to_json()
                for key, value in sorted(self.expectations.items())
            },
