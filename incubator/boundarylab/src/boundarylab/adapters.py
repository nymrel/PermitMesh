...
def _guest_environment(
    ...
    *,
    ...
    environment["BOUNDARYLAB_NETWORK_PROBES"] = (
        "1" if network_probes else "0"
    )
...
class VercelAdapter:
    ...
    def run(
        ...
        *,
        ...
        try:
            session_factory: Any = getattr(api_module, "session")
            network_policy: Any = getattr(
                sandbox_module,
                "NetworkPolicy",
            )
            create_sandbox: Any = getattr(
                sync_module,
                "create_sandbox",
            )
            with session_factory():
                with create_sandbox(
                    ...
                    execution_time_limit=timedelta(
                        seconds=(
                            authorization.limits.max_duration_seconds
                        )
                    ),
                    persistent=False,
