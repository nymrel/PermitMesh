...
def _configure_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="boundarylab",
        description=(
            "Authorized, provider-neutral sandbox conformance evidence."
        ),
    )
...
def _command_init(args: argparse.Namespace) -> int:
    ...
    if existing and not args.force:
        raise BoundaryLabError(
            "refusing to replace existing files: " + ", ".join(existing)
        )
...
def _command_probes(args: argparse.Namespace) -> int:
    ...
    if args.json:
        ...
        for record in records:
            print(
                f"{record['probe_id']}\t{record['category']}\t{record['description']}"
            )
    return 0
